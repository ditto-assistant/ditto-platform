...
- **Platform-operated screening worker:** private `ditto-assistant/ditto-screener`
...
(Swagger) and `/openapi.json`. The private screener additionally shares only the
...
rejects deterministic submission failures, and retries infrastructure failures
after lease expiry. A crate that does not compile never costs a full benchmark.
