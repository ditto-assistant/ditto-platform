...
class TestBenchmarkRolloutSettingsQueries:
    async def test_latest_is_none_when_empty(self, session: AsyncSession) -> None:
        assert await latest_benchmark_rollout_settings_revision(session) is None
    ...
    async def test_insert_and_read_latest(self, session: AsyncSession) -> None:
        ...
        row = await latest_benchmark_rollout_settings_revision(session)
