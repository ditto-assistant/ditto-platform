...
class ConfirmationJobRequest(BaseModel):
    """Signed request for one uncertainty-band challenger confirmation.

    The platform independently verifies that ``challenger_agent_id`` is the
    current raw leader, ``champion_agent_id`` is the KOTH incumbent, and the
    pair clears the flat margin but remains undecided only because its evidence
    is unpaired.  Binding both ids into the signature prevents a captured claim
    from being redirected to another scored submission.
    """
